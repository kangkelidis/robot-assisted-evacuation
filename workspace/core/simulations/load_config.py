"""
Responsible for loading the JSON configuration file, checkiing its validity,
...
from core.utils.paths import (CONFIG_FILE, NETLOGO_FOLDER, NETLOGO_HOME,
                              SCENARIOS_TEMP_FILE_RELATIVE_PATH)
...
def load_config(config_file_path):
    ...
    required_keys = ['netlogoModeName', 'scenarioParams', 'simulationScenarios']
...
def load_scenarios():
    ...
    for scenario_dict in config['simulationScenarios']:
        if scenario_dict.get('enabled', False):
            ...
            scenario_obj.build_simulations(config['netlogoModelPath'])
...
def save_scenarios(scenarios):
    ...
    try:
        with open(SCENARIOS_TEMP_FILE_RELATIVE_PATH, 'w') as temp_file:
            scenarios_dict = [{
                'name': scenario.name,
